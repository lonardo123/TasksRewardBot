require('dotenv').config(); const { Client } = require('pg'); const express = require('express'); const crypto = require('crypto'); const path = require('path'); const fs = require('fs'); // === إعداد قاعدة البيانات (Postgres Client) const client = new Client({ connectionString: process.env.DATABASE_URL, ssl: { rejectUnauthorized: false } }); // التقاط أخطاء غير متوقعة على مستوى العميل client.on('error', (err) => { console.error('PG client error:', err); }); // دالة لإنشاء/التأكد من الجداول والأعمدة (تنفيذ متسلسل لتجنب مشكلات multi-statement) async function ensureTables() { // أنشئ جدول users await client.query( CREATE TABLE IF NOT EXISTS users ( id SERIAL PRIMARY KEY, telegram_id BIGINT UNIQUE, balance NUMERIC(12,6) DEFAULT 0, payeer_wallet VARCHAR, created_at TIMESTAMPTZ DEFAULT NOW() ); ); // ✅ إنشاء / تعديل جدول user_videos ليتوافق مع جميع الحقول الجديدة await client.query( CREATE TABLE IF NOT EXISTS user_videos ( id SERIAL PRIMARY KEY, user_id BIGINT NOT NULL, title VARCHAR(255) NOT NULL, video_url TEXT NOT NULL, duration_seconds INT NOT NULL CHECK (duration_seconds >= 50), views_count INT DEFAULT 0, keywords TEXT, -- قائمة الكلمات المفتاحية بصيغة JSON viewing_method VARCHAR(50) DEFAULT 'keyword', -- طريقة العرض (keyword, direct, channel...) like VARCHAR(10) DEFAULT 'no', -- الإعجاب: yes / no / random subscribe VARCHAR(10) DEFAULT 'no', -- الاشتراك: yes / no / random comment VARCHAR(10) DEFAULT 'no', -- التعليق: yes / no / random comment_like VARCHAR(10) DEFAULT 'no', -- إعجاب بالتعليق: yes / no / random filtering VARCHAR(10) DEFAULT 'no', -- تصفية الحركة: yes / no daily_budget NUMERIC(12,6) DEFAULT 0, -- حد الميزانية اليومية total_budget NUMERIC(12,6) DEFAULT 0, -- حد الميزانية الإجمالية created_at TIMESTAMPTZ DEFAULT NOW() ); ); // أنشئ جدول earnings (مهيأ ليشمل watched_seconds, video_id, created_at) await client.query( CREATE TABLE IF NOT EXISTS earnings ( id SERIAL PRIMARY KEY, user_id BIGINT, source VARCHAR(50), amount NUMERIC(12,6), description TEXT, watched_seconds INTEGER, video_id INT, created_at TIMESTAMPTZ DEFAULT NOW() ); ); // أنشئ جدول withdrawals await client.query( CREATE TABLE IF NOT EXISTS withdrawals ( id SERIAL PRIMARY KEY, user_id BIGINT, amount NUMERIC(12,6), payeer_wallet VARCHAR, status VARCHAR(20) DEFAULT 'pending', requested_at TIMESTAMPTZ DEFAULT NOW(), processed_at TIMESTAMPTZ, admin_note TEXT ); ); // أنشئ جدول referrals await client.query( CREATE TABLE IF NOT EXISTS referrals ( id SERIAL PRIMARY KEY, referrer_id BIGINT, referee_id BIGINT, created_at TIMESTAMPTZ DEFAULT NOW() ); ); } // دالة لربط DB مع محاولة إعادة للاتصال عند الخطأ async function connectDB() { try { await client.connect(); console.log('✅ اتصال قاعدة البيانات ناجح'); // تأكد من الجداول await ensureTables(); console.log('✅ الجداول والأعمدة أنشئت أو موجودة مسبقًا'); } catch (err) { console.error('❌ فشل الاتصال بقاعدة البيانات:', err.message || err); // إعادة المحاولة بعد 5 ثوانٍ setTimeout(connectDB, 5000); } } // === السيرفر (Express) import express from "express"; import fs from "fs"; import path from "path"; import { fileURLToPath } from "url"; const app = express(); app.use(express.json()); // ضروري لتحديد المسار الكامل للمجلد public const __filename = fileURLToPath(import.meta.url); const __dirname = path.dirname(__filename); // الملفات الثابتة (public) app.use(express.static(path.join(__dirname, "public"))); // ✅ مسار /worker/start لعرض ملف Start.js مباشرة app.get("/worker/start", (req, res) => { const filePath = path.join(__dirname, "public", "assets", "js", "core", "Start.js"); if (fs.existsSync(filePath)) { res.type("application/javascript"); res.sendFile(filePath); } else { res.status(404).send("// ⚠️ لم يتم العثور على Start.js أو initWorker!"); } }); // =========================================== // ✅ مسار التحقق من العامل (Worker Verification) // =========================================== app.all("/api/worker/verification/", (req, res) => { // دعم GET و POST مع رد ثابت يطمئن الإضافة res.status(200).json({ ok: true, status: "verified", method: req.method, server_time: new Date().toISOString() }); }); app.get('/api/user/profile', async (req, res) => { const { user_id } = req.query; if (!user_id) { return res.status(400).json({ status: "error", message: "user_id is required" }); } try { const result = await client.query( 'SELECT telegram_id, balance FROM users WHERE telegram_id = $1', [user_id] ); if (result.rows.length > 0) { const user = result.rows[0]; return res.json({ status: "success", data: { user_id: user.telegram_id.toString(), fullname: User ${user.telegram_id}, balance: parseFloat(user.balance), membership: "Free" } }); } else { // إنشاء مستخدم جديد برصيد 0 await client.query( 'INSERT INTO users (telegram_id, balance, created_at) VALUES ($1, $2, NOW())', [user_id, 0] ); return res.json({ status: "success", data: { // ← ✅ تم إضافة "data:" هنا user_id: user_id.toString(), fullname: User ${user_id}, balance: 0.0, membership: "Free" } }); } } catch (err) { console.error('Error in /api/user/profile:', err); return res.status(500).json({ status: "error", message: "Server error" }); } }); app.get('/', (req, res) => { res.send('✅ السيرفر يعمل! Postback جاهز.'); });
