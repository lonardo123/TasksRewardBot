...
async function connectDB() {
  try {
    ...
    console.error('❌ bot.js: فشل الاتصال:', err.message || err);
...
await ctx.reply("✅ تم إرسال الإثبات، وسيتم مراجعته من الإدارة.");
      ...
      try { /* rollback handled by txClient */
} catch(_) {}
...
// جلب الإثبات والتأكد من أنه pending
    const subRes = await txClient.query('SELECT * FROM task_proofs WHERE id=$1 AND status=$2', [subId, 'pending']);
    ...
    // جلب سعر المهمة
    const taskRes = await txClient.query('SELECT price FROM tasks WHERE id=$1', [sub.task_id]);
    const price = parseFloat(taskRes.rows[0]?.price) || 0;

    // إضافة الرصيد للمستخدم (أو إنشاء صف جديد إن لم يكن موجوداً)
    const upd = await txClient.query('UPDATE users SET balance = COALESCE(balance,0) + $1 WHERE telegram_id = $2', [price, sub.user_id]);
    if (upd.rowCount === 0) {
      await txClient.query('INSERT INTO users (telegram_id, balance) VALUES ($1, $2)', [sub.user_id, price]);
    }
    ...
    await txClient.query('UPDATE task_proofs SET status=$1 WHERE id=$2', ['approved', subId]);
    ...
    try { 
      await bot.telegram.sendMessage(sub.user
        await txClient.query('COMMIT');
      } catch (err) {
        try { await txClient.query('ROLLBACK'); } catch(_) {}
        txClient.release();
...
مهمة (ID: ${sub.task_id}). المبلغ ${price.toFixed(4)}$ أُضيف إلى رصيدك.`); 
    } catch (_) {}
...
(async () => {
  try {
    ...
    process.once('SIGINT', () => {
      ...
      bot.stop('SIGINT');
      pool.end().then(() => console.log('🗄️ Postgres connection closed.'));
    });
