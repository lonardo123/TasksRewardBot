...
// ✅ هاندلر نصوص آمن للمستخدمين والأدمن — لا يسبب تعارض
bot.on('text', async (ctx, next) => {
  if (!ctx.session) ctx.session = {};

  const text = ctx.message?.text?.trim();

  // نصوص المستخدم العادي
  const userMenuTexts = new Set([
    '💰 رصيدك', '🎁 مصادر الربح', '📤 طلب سحب', '👥 ريفيرال',
    '📝 مهمات TasksRewardBot', '🔗 قيم البوت من هنا'
  ]);

  // نصوص الأدمن
  const adminMenuTexts = new Set([
    '📋 عرض الطلبات','📊 الإحصائيات',
    '➕ إضافة رصيد','➖ خصم رصيد',
    '➕ إضافة مهمة جديدة','📝 المهمات','📝 اثباتات مهمات المستخدمين',
    '🚪 خروج من لوحة الأدمن'
  ...
  // تجاهل النصوص إذا كانت في القوائم لتجنب أي تعارض
  if (userMenuTexts.has(text)) return next();       // نصوص المستخدم
  if (String(ctx.from.id) === process.env.ADMIN_ID && adminMenuTexts.has(text)) return next(); // نصوص الأدمن

  return next(); // أي نص آخر يعالج حسب الهاندلرات الأخرى
});
