...
// 🛠 أمر /admin - قائمة الأدمن مع تنظيم أفضل وبدون تغيير المسميات الأصلية
...
// 🏠 /start - تصحيح ترتيب الأزرار (الاستثمار بجانب الفيديوهات حسب طلب المستخدم)
bot.start(async (ctx) => {
  ...
  try {
    ...
    await ctx.replyWithHTML(
      ...
      Markup.keyboard([
        ...
        [t(lang, 'language'), t(lang, 'investment')], // ✅ الاستثمار بجانب الفيديوهات
        [t(lang, 'rate')],
...
// 📈 استثماري - تصحيح كامل باستخدام نظام الترجمة
bot.hears((text, ctx) => text === t(getLang(ctx), 'investment'), async (ctx) => {
  const userId = ctx.from.id;
  const lang = getLang(ctx);
  const url = `${MAIN_URL}/investment.html?user_id=${userId}&lang=${lang}`;
  await ctx.reply(
    lang === 'ar' ? "📈 افتح صفحة الاستثمار:" : "📈 Open your investment page:",
    {
      reply_markup: {
        inline_keyboard: [[
          { text: lang === 'ar' ? "📈 افتح الاستثمار" : "📈 Open Investment", url: url }
        ]]
      }
    }
  );
...
// 📈 إدارة الاستثمار (للأدمن) - تصحيح كامل
bot.hears('📈 إدارة الاستثمار', async (ctx) => {
  ...
  await ctx.reply(
    "📈 لوحة إدارة الاستثمار",
    {
      reply_markup: {
        inline_keyboard: [[
          { text: "⚙️ Open Admin Panel", url: url }
        ]]
      }
    }
  );
...
// ✅ موافقة الأدمن - تصحيح أخطاء كتابية في الكود الأصلي
